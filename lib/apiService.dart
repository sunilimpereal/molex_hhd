...
import 'package:molex_mobile/KittingIssance/models/kitting_put_model.dart';
...
//369200603
  Future<List<MaterialCodinatorScheduler>> getMaterialCordScheduelarData() async {
    ...
    if (response.statusCode == 200) {
      ...
      log(response.body);
      List<MaterialCodinatorScheduler> scheduleList =
          materialSchedualr.data.materialCodinatorSchedulerData;
      return scheduleList;
    } else {
      return [];
    }
  ...
  Future<List<BundleDetail>?> getPickedBundleDetail(String binId) async {
    var url =
        Uri.parse(baseUrl + "molex/material-codinator/material-codinator-picked-data?binId=$binId");
    var response = await http.get(url);
  ...
  Future<bool> pickedBundle(List<PutBundlePicked> bundleLIst) async {
    ...
    var response =
        await http.put(url, body: putBundlePickedListToJson(bundleLIst), headers: headerList);
  ...
  Future<List<BundlesRetrieved>?> getBundlesinBinStatus(
      PostgetBundleMaster postgetBundleMaster) async {
...
// }
  ...
  Future<bool> updateBundleKittingTransfer(List<UpdateBundleTransfer> bundleLIst) async {
    ...
    var response =
        await http.put(url, body: updateBundleTransferToJson(bundleLIst), headers: headerList);
  ...
  Future<bool> updateBundleKitting(List<UpdateBundleKitting> bundleLIst) async {
    ...
    var response =
        await http.put(url, body: updateBundleKittingToJson(bundleLIst), headers: headerList);
...
//Get kitting issuance list
  ...
  Future<bool> postKittingIssunce(List<PostKittingIssuance> postKittingIssuance) async {
    ...
    var response = await http.put(url,
        body: postKittingIssuanceToJson(postKittingIssuance), headers: headerList);
  ...
  Future<bool> postKittingIssuncePutMethod(List<KittingPutModel> kittingPutModelList) async {
    ...
    var url = Uri.parse(baseUrl + "molex/kitting");
    var response =
        await http.put(url, body: kittingPutModelToJson(kittingPutModelList), headers: headerList);
...
// localhost:9090/molex/bundlemaster?binId=1013&status=dropped&bundleId=1234&location=2345&finishedGoods=8756&cablePartNumber=9876&orderId=4567
  Future<List<BundlesRetrieved>?> getBundlesFgpartNo(
      {required PostgetBundleMaster postgetBundleMaster}) async {
    log(' URL /molex/bundlemaster/');
    var url = Uri.parse(baseUrl + '/molex/bundlemaster');
    var response = await http.post(url,
        body: postgetBundleMasterToJson(postgetBundleMaster), headers: headerList);
    ...
    if (response.statusCode == 200) {
      BundleDetailSts getBinDetail = bundleDetailStsFromJson(response.body);
      List<BundlesRetrieved> bundleList = getBinDetail.data.bundlesRetrieved
          .where((element) => element.bundleStatus == 'Kitting')
          .toList();
      if (bundleList.length == 0) {
        // Fluttertoast.showToast(
        //   msg: "No Bundles in BIN",
  ...
  Future<List<BundleTransferToBinTracking>?> postTransferBinToLocation(
    ...
    if (response.statusCode == 200) {
      try {
        List<BundleTransferToBinTracking> b =
